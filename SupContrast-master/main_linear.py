...
from networks.gac_ode import SupConResNet, LinearClassifier
...
def validate(val_loader, model, classifier, criterion, opt):
    ...
    with torch.no_grad():
        ...
        for idx, (images, labels) in enumerate(val_loader):
            ...
            # flops, params = profile(classifier(model.encoder(x)), inputs=(x,))
            # if idx == 0:
            #     print('flops  of ODE is %.2fG' % (flops/1e9))
            #     print('parameters  of ODE is %.2fG' % (params/1e6))
            loss = criterion(output, labels)
